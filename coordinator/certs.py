...
def generate_ca() -> None:
    ...
    _run([
        ...
        "-days", "3650", "-nodes",
        "-subj", "/C=CN/O=Tongrui/CN=Tongrui Root CA",
    ])
...
def issue_cert(node_id: str, is_server: bool = False) -> tuple[Path, Path]:
    ...
    ext = ["subjectAltName=DNS:coordinator"] if is_server else []
    cmd = ["openssl", "req", "-new", "-key", str(node_key), "-out", str(node_csr), "-subj", subj]
    _run(cmd)

    # Sign with CA
    sign_cmd = [
        "openssl", "x509", "-req",
        "-in", str(node_csr), "-CA", str(CERTS_DIR / "ca.crt"),
        "-CAkey", str(CERTS_DIR / "ca.key"),
        "-CAcreateserial", "-out", str(node_crt),
        "-days", "365",
    ]
    _run(sign_cmd)
...
def revoke_cert(node_id: str) -> None:
    ...
    _run([
        "openssl", "ca", "-revoke", str(node_crt),
        "-keyfile", str(CERTS_DIR / "ca.key"),
        "-cert", str(CERTS_DIR / "ca.crt"),
